...
// // const { obtenerPrestamosCliente } = require("../services/obtenerPrestamosCliente");
...
// router.get('/prestamos-resportes', async(req, res) => {
//     if (req.session.loggedin) {

//         const arrayPrestamosDB = await pool.query('SELECT * FROM prestamos ORDER BY fecha_creacion DESC ');


//         res.render("prestamos-activos", {
//             arrayPrestamos: arrayPrestamosDB,
//         });
...
// // Asumo que 'pool' y 'router' ya están definidos y configurados

// /**
//  * Función auxiliar para construir la consulta SQL dinámica basada en los filtros.
//  * @param {object} params - Los parámetros enviados desde el cliente (Front-end).
//  * @returns {{sql: string, values: Array}} - El query SQL y los valores para sanitización.
//  */
// function buildDynamicQuery(params) {
//     let sql = 'SELECT * FROM prestamos WHERE 1=1'; // 1=1 permite añadir condiciones fácilmente
//     const values = [];

//     // --- FILTROS DE CARTERA (Asumo que el campo de la BD se llama 'cartera') ---
//     // Si 'Incluir todas las Carteras' NO está marcado, filtrar por el valor de 'cartera'
//     if (!params.incluirTodasCarteras && params.cartera) {
//         // En un escenario real, 'cartera' podría ser un ID o un nombre, ajusta el campo según tu BD
//         sql += ' AND cartera = ?';
//         values.push(params.cartera);
//     }

//     // --- FILTROS DE SUCURSAL (Asumo que el campo de la BD se llama 'sucursal') ---
//     // Si 'Incluir todas las Sucursales' NO está marcado, filtrar por el valor de 'sucursal'
//     if (!params.incluirTodasSucursales && params.sucursal) {
//         // En un escenario real, 'sucursal' podría ser un ID o un nombre, ajusta el campo según tu BD
//         sql += ' AND sucursal = ?';
//         values.push(params.sucursal);
//     }

//     // --- FILTRO DE TIPO DE PRÉSTAMO (El campo de la BD es 'tipo') ---
//     // Si 'Incluir todos los tipos' NO está marcado, filtrar por el valor de 'tipo'
//     if (!params.incluirTodosTipos && params.tipo) {
//         sql += ' AND tipo = ?';
//         values.push(params.tipo);
//     }

//     // --- FILTRO DE ESTADO ('radioTipo' - Normal, Legal, Incobrable, Todos) ---
//     // Asumo que tienes un campo en la BD que determina el estado (p. ej., 'estado_prestamo')
//     // El campo de la BD de tu tabla 'prestamos' no tiene 'estado', así que asumo un campo llamado 'estado_prestamo'
//     // que almacena 'Normal', 'Legal', 'Incobrable'. Si no existe, debes añadirlo o usar una lógica diferente.
//     if (params.radioTipo !== 'Todos') {
//         sql += ' AND estado_prestamo = ?';
//         values.push(params.radioTipo);
//     }

//     // --- FILTRO DE FECHA DE CORTE ('fechaCorte') ---
//     // Generalmente, un reporte de cartera al corte incluye préstamos cuya fecha de creación
//     // o apertura sea menor o igual a la fecha de corte. Usaremos 'apertura' del campo de tu tabla.
//     if (params.fechaCorte) {
//         // Busca préstamos con fecha de apertura ANTES o IGUAL a la fecha de corte.
//         sql += ' AND apertura <= ?';
//         values.push(params.fechaCorte);
//     }
...
// // --- NUEVA RUTA PARA PROCESAR EL REPORTE ---
// // --- RUTA CORREGIDA PARA PROCESAR EL REPORTE ---
// // router.post('/api/prestamos-reportes-filtrados', async(req, res) => {
// //     // 1. Verificar sesión
// //     if (!req.session.loggedin) {
// //         return res.status(401).json({ success: false, message: 'Debe iniciar sesión para acceder a este recurso.' });
// //     }

// //     const params = req.body;
// //     let conn; // Declarada fuera para que 'finally' pueda acceder a ella

// //     try {
// //         // 2. INTENTO DE OBTENER LA CONEXIÓN (La fuente de tu error)
// //         // Si esta línea falla (ej. credenciales, DB no existe), 'conn' será undefined.
// //         // Si falla, el bloque 'catch' se activará.
// //         conn = await pool.getConnection();

// //         // 3. Si la conexión es exitosa, se procede a construir y ejecutar el query
// //         const { sql, values } = buildDynamicQuery(params);

// //         // 4. Ejecutar la consulta (Aquí es donde ocurría el error si 'conn' era undefined)
...
// //             success: true,
// //             data: rows,
// //             message: `Reporte generado con éxito. ${rows.length} préstamos encontrados.`,
// //         });

// //     } catch (error) {
// //         // Manejo de CUALQUIER error (conexión, consulta, o lógica)
// //         console.error('Error al generar el reporte de préstamos:', error);

// //         // El error de TypeError: Cannot read properties of undefined (reading 'execute') 
// //         // y el TypeError: cb is not a function serán capturados aquí.
// //         return res.status(500).json({
// //             success: false,
// //             message: 'Error interno del servidor al consultar la base de datos. Verifique la conexión.',
// //             error: error.message
// //         });
...
//     res.redirect(`/Solicitudes/editar-solicitud/${req.params.id}`);
// });

// //GUARDAR PRESTAMO DE SOLICITUD 
// router.post('/prestamo_cliente/:id', async(req, res) => {
//     const id = req.params.id;
...
// router.post('/Solicitudes/agregar-observacion-solicitud/:id', async(req, res) => {
//     // const id = req.params.id;
//     console.log(req.params.id)
//     console.log('entro a agregar-observacion-solicitud')
...
// //IMPRIMIR SOLICITUD************
// router.get("/Solicitudes/imprimir-solicitud/:id", async(req, res) => {
//     if (req.session.loggedin) {
...
// //IMPRIMIR CONTRATO************
// router.get("/Solicitudes/imprimir-contrato/:id", async(req, res) => {
//     if (req.session.loggedin) {
